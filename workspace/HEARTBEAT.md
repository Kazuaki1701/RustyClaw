...
Check for manually registered bookmarks in KaraKeep.

1.  Use the Karakeep API to fetch recent bookmarks.
2.  **Filter by Tags**: Identify items tagged with `_bookmarked` or `_doitlater`.
3.  **Action Item Analysis**: For `_doitlater`, extract actionable tasks and prepare reminders.
4.  **Knowledge Extraction**: For `_bookmarked`, evaluate applicability to K-sama's environment and Obsidian.
5.  If a valuable use case or configuration is identified, prepare a summary for the next briefing or notify immediately if it's a critical tool.

Check `lastChecks.karakeepPatrol` — if it's been less than 12 hours, skip this step.

## Step 8: Response

If no critical issues were found and no notifications were sent in this run:
- **Respond with `HEARTBEAT_OK`** (internal pipeline signal).
- **Do NOT post `HEARTBEAT_OK` to Discord.** (Per user directive: "Heartbeat OK の場合は、Discord への報告不要")

If critical issues were found or notifications were sent, provide a concise summary as your final response.
