...
> [!NOTE]
> **ステータス**: `[HISTORICAL]` (初期実装ロードマップ・決定事項アーカイブ)  
> **最終更新日**: 2026-05-28  
> **対象コード**: プロジェクト全体  
> **備考**: 本ロードマップは初期の進捗・決定事項を記録したアーカイブです。GeminiClaw 参照ソースマッピングの最新情報は [91_geminiclaw_comparison.md](file:///home/kazuaki/Projects/RustyClaw/docs/specs/91_geminiclaw_comparison.md) へ集約・移転されました。
...
13. **Google Workspace 連携の `gws` への一本化**
    複数プロセス常駐を排除するため、Go製 `googleworkspace/cli` (`gws mcp`) に一本化します。
14. **Karakeep および Obsidian のインプロセス・Rust 直実装**
    外部の Node/Python 常駐プロセスを全廃し、`rustyclaw-tools` の Rust ネイティブツール (`reqwest` 経由) として通信を直実装します。
15. **Markdown RAG における `stn/rqmd` の採用**
    Obsidian の意味検索による知識ベース拡張には、Rust 製の超軽量・ローカル検索エンジン `stn/rqmd` を `rustyclaw-tools` の CLI 連携として採用します。
