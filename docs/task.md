...
- [ ] **Phase 26-1: McpClientHandler Auto-Reconnect**: MCP サーバー（汎用 stdio）の接続断を検知し指数バックオフで再スポーン。着手条件: 実運用で接続断が発生した時点。対象: `crates/rustyclaw-tools/src/`（`McpClientHandler` / `ToolServerHandle`）。`10_mcp.md §2.1` 参照
