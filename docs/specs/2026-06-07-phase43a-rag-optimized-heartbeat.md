...
| `crates/rustyclaw-agent/src/lib.rs` | `execute_heartbeat`: `heartbeat_top_k` デフォルトを 4 に変更 |
...
`execute_heartbeat()` 内の `unwrap_or(2)` を `unwrap_or(4)` に変更する:
...
Step 2・Step 6 の専用クエリも同じ `hb_top_k` を使用するため、自動的に 4 に統一される。
...
| `heartbeat_top_k` デフォルト 2 | 4 に引き上げ |
