...
rustyclaw-agent (lib) (Pipeline)
    ...
    │   └── rig-core rmcp (McpClientHandler 経由の外部 MCP 接続)
    └── PublishResponse
        ↓
rustyclaw-providers (lib) (LlmProvider trait)
    └── OpenAiCompatProvider (reqwest + SSE、ToolCall 対応。HTTP直接接続に一本化)
        ↓
rustyclaw-storage (lib)
    ...
    └── SqliteStore
        ...
        ├── seen_items テーブル（既読アイテム管理・重複通知防止）
...
| MCP クライアント | `rig-core` 移行 | `rig-core` の `rmcp` フィーチャーを使用し、McpClientHandler で接続 |
| 安全な書き込み | `tempfile` | 原子性書き込み（電源断対策） |
| systemd連携 | `sd-notify` | WatchdogSec 連携用 |
| 設定暗号化 | `pbkdf2` + `aes-gcm` | `vault.enc` の AES-256-GCM 暗号化・復号 |
...
### ① Google Workspace 連携の軽量化とオンデマンド実行
- **決定事項**: 従来の Node.js ベースの MCP サーバーや、常駐型の `gws mcp` サーバー (MCP経由) を廃止。
- **代替案**: スクリプト (`calendar-ops.sh` / `506_get-gmail.sh` など) を経由して `gws` CLI コマンドをオンデマンドに呼び出す方式に移行。
- **効果**: 常駐プロセスを全廃し、必要な瞬間だけ `gws` コマンドを実行してメモリ消費・起動負荷をゼロ化。
...
### ③ 長期記憶の RAG 検索の統合
- **仕様**: `crates/rustyclaw-agent` および `crates/rustyclaw-storage` にて、長期記憶ファイル (`MEMORY.md`) のバレット行をチャンク化してベクトル化し、SQLite の `memory_embeddings` テーブルに保存・検索する RAG 機喚を統合。外部プロセスや Node.js 依存のない Rust ネイティブなベクトル検索を実現。
