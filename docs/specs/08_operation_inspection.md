...
**見るべき点**:
- `activityReview`（最終 Heartbeat 実行時刻）が現在時刻から 20 分以内か
...
**見るべき点**:
- 10 分間隔で Heartbeat が記録されているか（20 分以上の空白は異常）
...
**判定基準**:
- `empty_assistant` が全 assistant ターンの 80% 超 → gmn が空応答を返している（`--no-agent` × MCP ツール問題）
...
grep -oP '\[\K[0-9]{2}:[0-9]{2}:[0-9]{2}(?=\] Heartbeat Patrol)' \
  ...
  | awk 'NR>1 {
      ...
      if (diff < 0) diff += 1440;
      if (diff > 15) printf "GAP %s -> %s (%d min)\n", prev, $0, diff;
    }
    {prev=$0}'
```

**判定基準**:
- 15 分超のギャップがある場合 → gmn_sem 待機（Daily Summary 等との競合）または rate limit の疑い
...
```bash
# flush カウント・タイムスタンプ
sqlite3 workspace/memory.db "SELECT key, value FROM kv_store WHERE key LIKE 'flush%' ORDER BY key;"

# lastUserContact
sqlite3 workspace/memory.db "SELECT key, value FROM kv_store WHERE key = 'lastUserContact';"
...
# gmn 起動ログ（RUST_LOG=debug 時のみ）
grep "gmn spawn\|gmn exit" ~/.rustyclaw/rustyclaw.log | tail -20
...
症状: Heartbeat が 20 分以上止まっている
  ...
  │    └─ 原因: gmn_sem を Daily Summary 等が長時間占有
  │       └─ 対処: Daily Summary の実行タイミング調整
  └─ ログに "rate-limit" がある
       └─ 対処: GMN_MAX_RETRIES 値の見直し・指数バックオフ実装検討
...
症状: assistant 応答がすべて空
  └─ 原因: モデルがテキストなしでツール呼び出しのみ生成
     └─ 確認: gmn の stdout が空で返っているか RUST_LOG=debug で確認
```
...
| 確認項目 | コマンド | 正常基準 |
|---|---|---|
| Heartbeat 最終実行 | `cat workspace/memory/heartbeat-state.json` | 20 分以内 |
...
| Heartbeat ギャップ | 3-3 のスクリプト | ギャップなし |
| 空応答率 | 3-2 のスクリプト | cron-heartbeat 以外は 20% 以下 |
| SQLite flush カウント | `sqlite3 workspace/memory.db ...` | flush_ts が最近更新されている |
