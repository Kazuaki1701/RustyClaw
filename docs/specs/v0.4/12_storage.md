...
- **Phase 27-2: Cron セッション・ログの自動プルーニング**
  - 古い `cron:` 実行ログやセッションファイル（`sessions/cron-*.jsonl`）を自動消去するクリーンアップ機構。14 日 TTL を基準に、バッチ実行時に対象ファイルを削除し RAG インデックスからも除去する。対象: `crates/rustyclaw-gateway/src/cron.rs`。

- **Phase 43-F: 自己進化型 RAG（RAG Flywheel）**
  - 本番稼働時の外部コマンド実行エラーや外部 API（気象・Discord 等）の呼び出しエラーが発生した際、その解決プロセス（自動リトライ・フォールバック手順）を自動的に RAG へ登録する。次回同様のエラーを検知した際、事前に解決策をロードしてシステムダウンを防止する。エラー発生・解決時のみ embedding を生成するため、定常負荷は増えない。

- **Phase 43-G-1: KaraKeep ブックマークのインデックス化**
  - KaraKeep サーバーから定時バッチでブックマーク（タイトル・タグ・URL）を取得してインジェスト。対話中に「保存した記事」を自然言語で検索・提示できるようにする。テキスト量が少ないため RPi4 への負荷も極めて低い。

- **Phase 43-G-2: Obsidian 増分インジェストとパーソナルナレッジ参照**
  - Obsidian の指定ディレクトリ（または `#rag` タグ付きノート）をスキャン。深夜のアイドル時間帯に差分のみを増分 embedding してセカンドブレイン化する。RPi4 の CPU 負荷集中を避けるため Lane B で `spawn_blocking` 実行。

- **stn/rqmd によるローカル知識ベース RAG 構築**（Phase 13 積み残し）
  - stn/rqmd ツールを活用したローカル知識ベースの RAG 構築。ユースケースが明確になった時点で詳細設計する。

### Embedding 検索の高度化

- **時間情報の有効化（ハイブリッド検索・再ランク付け）**
  - 「今日」「今週」などの時間的キーワードを含むクエリに対して、Embedding 検索対象範囲を更新日時などのメタデータでフィルタリングするハイブリッド検索。
  - 検索結果に対して時間的近接度スコアを加算し、新しい情報を上位に引き上げる再ランク付け機構。現行の `search_similar_with_decay` の時間減衰アプローチを発展させ、クエリの時間意図を明示的に解釈する。
