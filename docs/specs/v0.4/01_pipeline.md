...
- **ISSUE-22: `gmn_sem` capacity の config 化と書き込み直列化の責務分離**（前提: capacity 引き上げが効果的と判断できる実測データが得られた時点。メモリ `project_user_sem_concurrency` 参照）
  - 現在ハードコードされている `limit=1` を設定ファイルで調整可能にする。

- **Phase 25-2: セマフォ取得の安全待機タイムアウト**
  - `crates/rustyclaw-gateway/src/lib.rs` のセマフォ取得処理に `tokio::time::timeout` を導入。一定時間内に Permit が得られない場合は警告ログを出力し、該当ジョブを破棄または再キューイングする。

- **Phase 25-5: 実行ステップのチェックポイント化と Lossless Resume**（GeminiClaw ギャップB）
  - パイプライン実行中にエラー（送信失敗等）が発生した際、LLM API の再呼び出しを行わずに失敗したフェーズから再試行できる中間状態の永続化と復旧機構。

### ContextBuilder / context window 最適化 `[v0.4 対応予定]`

- **context window サイズに応じた動的コンテキスト調整**
  - モデル毎の context window サイズ（`LlmConfig` から取得）に応じてセッション履歴・注入コンテキスト量を動的調整する。70/20/10 予算分割（§02_memory.md §5.3）を実装ベースに昇格させる。
  - 関連実装: **Heartbeat Digest**（Heartbeat 実行前に増分セッションダイジェスト生成）・**Session-level Summary**（アイドル後サマリー生成＋ `ctx_index` 登録）を段階的に導入。
  - 参照実装: GeminiClaw `src/geminiclaw/src/agent/session/heartbeat-digest.ts` / `summary.ts`

### CronService の拡張候補

- **Phase 27-3: 1 回限り (deleteAfterRun) ジョブの自動削除**
  - 実行完了後に `cron.json` から該当ジョブ定義を自動削除し、アトミックに更新保存する。`at` ジョブ（1 回限り実行）を使い捨てとして扱う。
