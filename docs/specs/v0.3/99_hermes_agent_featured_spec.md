...
> **備考**: `docs/specs/00_rustyclaw.md` を置き換え。Hermes Agent 拡張を統合した完全仕様。
...
| GeminiClaw (TypeScript) | メモリ 3 層・Heartbeat・Session Continuation 設計の参照 |
| Hermes Agent (Nous Research) | 自己改善 Skills・3 層メモリ制約・自己監査ループの参照 |
...
### 8.3 AGENTS.md における Heartbeat 応答規約
...
`27.5°C↑` や `1250ppm↑` という文字表現を読むだけで、ステートレスな LLM が「部屋が急速に暑くなり、空気も澱んできている」という時間の流れと変化の兆候を 1 発の推論ターンで理解できる。
...
```bash
# 本番
cd production/config && ln -sfn config.release.json config.json
# デバッグ
cd production/config && ln -sfn config.debug.json config.json
```
