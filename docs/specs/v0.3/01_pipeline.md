...
### ContextBuilder / リクエストペイロードの最適化

- **リクエストペイロードの最適化調査と実施**（Phase 44-6 着手前の前提作業）
  - LLM へ送信するコンテキスト長・メッセージ数・system prompt のさらなる削減余地を調査。現状 30 秒程度の応答遅延が残存しており、具体的な削減施策を設計・実施して許容レベルまで改善する。
