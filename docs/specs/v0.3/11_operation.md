...
**判定基準**:
- `empty_assistant` が全 assistant ターンの 80% 超 → LLM プロバイダが空応答、またはツール呼び出しループ制限（5回）に達した可能性
...
**判定基準**:
- 35 分超のギャップがある場合 → Lane B セマフォ待機（Daily Summary 等との競合）または rate limit の疑い
...
症状: チャットに ```json ... ``` ブロックが送信される
  └─ 原因: --no-agent モードで MCP ツール呼び出し JSON が漏出
     └─ 対処: AGENTS.md から不要なツール指示を削除（応急）

症状: Heartbeat が 40 分以上止まっている
  ├─ heartbeat-state.json の activityReview が古い
  ├─ ログに "timed out waiting for permit" がある
  │    └─ 原因: Lane B セマフォの空きスロット不足（Daily Summary 等との一時的な競合）
  │       └─ 対処: 各ジョブの実行時刻スケジュールの見直し・調整
  └─ ログに "rate-limit" がある
       └─ 対処: 指数バックオフによる自動クールダウン完了を待つ

症状: cron-heartbeat.jsonl が 1MB 超
  └─ 原因: 固定 session_id による履歴の無制限蓄積
     └─ 対処: ログプルーニング（Phase 27-2）参照
...
- [`09_dashboard.md`](09_dashboard.md) — Web Dashboard・管理用 API 仕様
