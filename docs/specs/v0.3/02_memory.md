...
### 5.3 70/20/10 コンテキスト戦略 `[v0.4 対応予定]`

コンテキストウィンドウを「対話バッファ(70%)」「動的文脈(20%)」「人格(10%)」に厳格に切り分け、各枠に予算上限を設ける方式。  
現状は Sliding Window ローテーションで代替。v0.4 で context window 最適化の一環として実装予定（`v0.4/00_rustyclaw.md §4.2・§7 #26` 参照）。
...
- **ContextBuilder 予算管理**: モデル毎の context window サイズから各枠の token 上限を算出し動的注入量を調整
