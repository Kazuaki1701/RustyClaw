...
- 処理内容：
  - 各種人格・コンテキストファイルの読み込みとシステムプロンプトへの結合。
    - **最大3,000文字圧縮（Phase 44-2）**: `SOUL.md` と `USER.md` は重要な内容のみを抽出し、最大 3,000 文字に圧縮してシステムプロンプトに注入します（`MAX_CONTEXT_CHARS_PER_FILE`）。これにより、不要な長文を除外しながら本質的な人格・プロフィール定義を保持し、リクエストサイズを削減します。
...
### プロバイダファクトリ
`config.json` から読み込まれた `LlmModelConfig` に基づいて具象プロバイダを生成します。
...
|---|---|---|
| `"openai"` | ✅ 実装済み | OpenAI 互換 API（reqwest + SSE、`ToolCall` 対応。HTTP直接接続に一本化） |
| `"gmn"` | ⚠️ デバッグ用 | gmn CLI サブプロセス（※本番の `create_provider` からは除外され、テスト・デバッグ用のみ） |
...
> [!NOTE]
> **現在非推奨**: 以下の `GmnCliProvider` 仕様は、当初開発された `gmn` CLI との統合設計です。現在は HTTP ストリーミング接続（`OpenAiCompatProvider`）がデフォルトとなっており、CLI サブプロセス呼び出しはデバッグおよび限定的なテスト環境でのみ機能します。
...
- **ダンプファイル**:
  - `last_request.json`: LLMに送信された最新のシステムプロンプト、会話履歴、利用可能なツール定義を含む生のJSON。
    - **Phase 44-2 コンパクト版追加**: タイムスタンプ付きダンプに加えて、ファイルサイズを **< 5 KB** に抑えたコンパクト版 `last_request.json` を毎回更新します。このコンパクト版は、SOUL.md / USER.md の圧縮（最大 3,000 文字）、会話履歴の先頭・末尾要約、ツール定義名の列挙のみなど、本質的な情報を保持しながら詳細を省いた形式です。本番環境でのネットワーク・ディスク負荷軽減に有効です。
...
| **`rustyclaw-config`** | `INFO`<br>`DEBUG` | ・`config.json` のロード成功、暗号化シークレット（`vault.enc`）の復号完了。<br>・環境変数による設定値の動的オーバーライド検出。 |
