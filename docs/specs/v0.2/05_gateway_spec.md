...
#### ⑤ WebPreviewServer (HTTPサーバー)
`axum` および `tower-http` を用いた非同期静的ファイル配信サーバー。デフォルトでポート `4000` で待受。
`workspace/previews/` 配下の HTML やアセット等の静的ファイルを安全にサービングします。また、起動時に Tailscale IPv4、ローカル IP などを自動判定し、エージェントがプレビューファイルを書き込んだ際 (`WorkspaceWriteTool`) にクリック可能なプレビュー用 URL（`http://<tailscale-ip>:4000/<filename>`）を提示する仕組みを提供します。
...
| 制御項目 | 値 | 役割 |
|---|:---:|---|
| **gmn 統合枠** (`gmn_sem`) | **4** | 全 gmn プロセス（user 対話 / bg heartbeat / flush_memory）を一本化。同一セッションの共有ファイル競合は `rustyclaw-storage` 内の「インプロセス非同期パスロック (`PATH_LOCKS`)」で排他制御し、データ破損を完全防止。[^gmn_sem] |
| **意図した最大同時 gmn 数** | **4** | 統合セマフォ容量を 4 に設定。 |
...
- **仕組み**: 全ての gmn プロセス起動を単一の `gmn_sem`（容量4）で制御します。
  - `gmn_sem`: 同時許可数 **4**（user 対話 / bg heartbeat / flush_memory を一本化）
- **統合・スケーリングの背景**: かつて（〜2026-05-28）は `user_sem` / `bg_sem` / `flush_sem` を分離していましたが、共有ファイルへの並列書き込み競合リスクを避けるため一時的に直列化（容量1）しました。その後、`rustyclaw-storage` 側に `RwLock` マップを用いた「インプロセス非同期パスロック (`PATH_LOCKS`)」を導入し、書き込みを完全に排他制御できるようにしたことで、安全に容量を **4** へ拡張して並行処理能力を向上させました。[^gmn_sem]
- **Antigravity 2.0 対応**: 最大同時 gmn 数を 1 → **4** に拡張。

[^gmn_sem]: **解決済み（2026-06-06）**: `gmn_sem > 1`（並列化復活）に伴う共有ファイル（`MEMORY.md` / `USER.md` / 日次ログ）の競合防止策として、`rustyclaw-storage` に `once_cell` と `RwLock` マップを用いた「インプロセス非同期パスロック (`PATH_LOCKS`)」を実装完了。

[^mcp_heartbeat]: **解決・緩和済み（2026-06-06）**: MCP ツール統合に伴い Heartbeat の実行時間が延伸した際の対策として、`gmn_sem` の容量を **4** に拡張しました。これにより、Heartbeat 実行中であっても他のレーンのユーザー対話が長時間待機状態になるのを回避・緩和しています。
