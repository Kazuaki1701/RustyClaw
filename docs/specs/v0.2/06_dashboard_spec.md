...
- **チャット機能**: 対話セッション ID は `"http-dashboard-{timestamp}"` 形式（ナノ秒精度のタイムスタンプ）として生成され、ブラウザからの入力履歴が蓄積されます。これにより日別のローテーションではなく、衝突の可能性が排除され、ログ肥大化が防止されます。
  - **タイムアウト設定**: サーバー側は `CHAT_TIMEOUT_SECS = 120` (120秒)で設定。クライアント(JS)側の `CHAT_TIMEOUT_MS = 120_000` (120秒)にて、タイムアウト時は自動リトライ 1 回を行う。
  - **応答キャッシュ**: LLM からの最終応答を 5 分間キャッシュし、同一メッセージの再送時は即座に返却して体感レスポンスを向上させる。
...
## 4. ダッシュボードチャットの RAG 連携仕様 (Phase 41-1)

ダッシュボードチャットからメッセージ（`/chat` への POST）が送信された際、単なる一問一答にとどまらず、実運用での出来事に基づいた会話を行えるよう、以下の RAG 連携および動的コンテキスト拡張が適用されます。

### ① タイムスタンプベースのセッション分離
対話のセッション ID は `"http-dashboard-{timestamp}"` 形式で生成され、ナノ秒精度のタイムスタンプに基づく一意なセッションとして扱われます。これにより、日付ローテーションに依存せず、衝突リスクが排除され、ログ肥大化が防止されます。

### ② heartbeat-digest.md の動的注入
ダッシュボードからのチャットセッションであるとサーバー（`execute_with_tools`）が検知した場合、最新のバックグラウンド自発行動ログの要約である `workspace/memory/heartbeat-digest.md` が存在すれば、システムプロンプトのコンテキスト（`## Latest Heartbeat Digest` セクション）として自動的に動的注入されます。
これにより、ユーザーは「今日のパトロール結果はどうだった？」などの問いかけをダッシュボード上でシームレスに行うことができます。

### ③ ダッシュボード専用 RAG top_k (dashboard_top_k) の適用
ダッシュボードからのチャットでは、設定ファイル上の `embedding.dashboard_top_k`（標準推奨値: `8`）で指定された件数が RAG 検索の取得件数（`top_k`）として適用されます（設定がない場合は標準の `top_k: 5` がフォールバックされます）。
これにより、過去に実行された cron サマリーや過去のダッシュボード会話ログなどから、より広範に関連する事実を取得して対話に役立てることができます。

### ④ cron セッションサマリーの自動 RAG インジェスト
以下のホワイトリストに登録された定時 cron ジョブが正常完了した際、gateway 内のメッセージバスを介して `IncomingMessage`（`cron:session-summary:<session_id>`）がバックグラウンド優先度で発行され、その実行サマリーが自動的にベクトルデータベース（`memory.db`）へインジェストされます。
