...
## 開発タスクチェックリスト
- [ ] **短期改善の実装 (Phase 52-1)**:
  - [ ] `memory flush` 時のデリミタを XML タグ化 (`<mem>`, `<log>`) に変更し、パースロジック (`extract_delimited_block`) をアップデート
  - [ ] システムプロンプトを高密度な日本語スリムプロンプトに圧縮
- [ ] **中期改善の実装 (Phase 52-1)**:
  - [ ] 会話履歴注入前にシステムメッセージやエラー、一時的な進捗文字列などを正規表現でクレンジングする前処理を実装
- [ ] **長期改善の実装 (Phase 52-1)**:
  - [ ] `MEMORY.md` をセクションまたはバレット単位で分割して MCP `context-mode` (SQLite/ベクトルDB) へ `ctx_index` 登録するバッチの実装
  - [ ] `ctx_search` を用いて、会話内容に関連するメモリのみを動的ロードする RAG 連携の実装
- [ ] **ドキュメント整理・DoDの達成 (Phase 52-1)**:
  - [ ] ドキュメント整理（仕様書同期・本計画書/検証レポートのアーカイブ退避・task.md のクレンジング）
...
1.  **MCP context-mode へのセクション分割登録**:
    *   `MEMORY.md` を「ユーザー基本情報」「仕事の文脈」「趣味・好み」といったセクション単位、あるいは箇条書き1行単位のチャンクに分割し、**MCP `context-mode`（`ctx_index` ツール経由）の SQLite / ベクトルデータベースに蓄積・同期**します。
2.  **動的なコンテキスト注入 (RAG)**:
    *   直近の会話内容から関連するキーワードやベクトルを抽出し、**MCP `context-mode` の `ctx_search` ツールを実行して、現在の会話に関係のあるセクションのみを動的に抽出・注入**します。
    *   メモリの更新時は、変更のあったセクションのみを LLM にマージ・書き換えさせ、システム側で該当セクションのみをアップデート（`ctx_patch` もしくは `ctx_index` による上書き）して再結合します。
