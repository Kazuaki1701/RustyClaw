...
### 2.4 ユースケースD：HomeAssistant センサー履歴の動的集計・状態探索
- **背景**: ユーザーがエージェントに対して「最近の室温の推移はどう？」「消し忘れているライトや異常なデバイス（`unavailable`）はある？」と尋ねた場合。
- **解決策**:
  1. **センサー履歴のインプロセス集計**: 数日〜数週間分のセンサー履歴（JSON/CSV）から、「1時間ごとの平均値」や「閾値（CO2 > 1000等）を超えたイベントの発生時刻とピーク値」をサンドボックス内のスクリプトで集計し、数行のMarkdownテーブルに濃縮して還流します。
  2. **エンティティ情報のノイズ排除**: HA の全エンティティ状態（数百KBの生JSON）を API から取得する際、サンドボックス内で `jq` 等により「現在オンのデバイス」や「`unavailable` 状態のデバイス」のみをフィルタ抽出し、余分なメタデータを除外して還流します。

#### スクリプトプロトタイプ (Tool Call - 異常デバイスの抽出):
```json
{
  "name": "ctx_execute_file",
  "arguments": {
    "filePath": "production/workspace/memory/ha-state.json",
    "script": "jq '[.[] | select(.state == \"unavailable\") | {entity_id, friendly_name}]' \"$FILE\""
  }
}
```

#### 削減期待値:
- **生データサイズ**: 300 KB (全エンティティの生状態JSON ≒ 75,000 トークン)
- **抽出後サイズ**: 800 B (異常デバイスリスト ≒ 200 トークン)
- **コンテキスト削減率**: **99.7%**
