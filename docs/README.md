...
*   **Git pre-push フックによる事前検証 (Enforced Pre-Push Verification)**:
    *   プッシュ前に必ずローカルで Clippy 警告およびテストが成功することを確認してください（`.git/hooks/pre-push` フックによって自動的に強制されます）。
    *   警告の無視やテストエラーがある状態でのプッシュをバイパスするために `--no-verify` フラグを使用することは一切禁止します。
