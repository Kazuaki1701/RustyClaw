...
| 制御項目 | 値 | 役割 |
|---|:---:|---|
| **ユーザー同時枠** (`user_sem`) | **2** | ユーザー対話セッションが同時に使用できる gmn スロット数。 |
...
| **意図した最大同時 gmn 数** | **4** | user(2) + bg(1) + flush(1)。 |
...
- **Antigravity 2.0 対応**: 合計最大 4 並列 gmn。`GMN_MAX_RETRIES=0` で内部リトライを無効化しているため最悪バーストは 4 リクエスト/秒程度に抑制される。
