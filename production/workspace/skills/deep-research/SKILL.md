...
Search result snippets are only summaries. **Read the full text of the top 3–5 results using `web_fetch`**.

Use `web_fetch` for all pages — both static and dynamic content.
...
Save to `memory/research-YYYY-MM-DD-{topic}.md` using `workspace_write` (date from `[now: ...]` in system prompt):
