...
# VS CodeとGhosttyの高度な連携手法

## TL;DR
VS Code 2026.2の「外部ターミナル・ディープインテグレーション」機能により、Ghostty等の外部ターミナルとエディタ間での環境変数やコマンド履歴のリアルタイム同期が可能になりました。Windows環境でもWSLgを活用することで、WSL上のGhosttyをVS Codeから直接呼び出し、シームレスな開発環境を構築する具体的手順を解説しています。

## Topics
- **ディープインテグレーション**: VS Code 2026.2で導入された、外部ターミナルとの双方向コンテキスト同期やインライン描画プロトコルについて解説しました。
- **Ghostty連携設定**: VS Codeの外部ターミナルとしてGhosttyを指定し、ショートカットやコマンドパレットから機能を有効化する手順を提示しました。
- **WSL環境でのGUI連携**: Windows上のVS CodeからWSLg経由でLinux版Ghosttyを起動し、Windowsデスクトップ上で利用する方法と注意点をまとめました。

## Key Decisions
- 外部ターミナルのデフォルト実行ファイルとして'ghostty'を設定することを推奨
- WSLリモート接続時の設定項目としてterminal.external.linuxExecの活用を決定
