...
## TL;DR
Fish shellの各環境（WSL, RPi, Docker）での利用可能性と、最新AIモデルGemma 4をローカル運用するためのハードウェア要件を調査しました。既存のQNAP NAS環境での運用可能性についても具体的にアドバイスしています。

## Topics
- **Fish shell 導入調査**: WSL、Raspberry Pi、Dockerの各環境でFish shellが問題なく利用可能であることを確認し、導入方法を提示しました。
- **Gemma 4 スペック要件**: 2026年4月にリリースされたGemma 4の各モデル（E2B, E4B等）に必要なメモリ量と特徴をまとめました。
- **既存環境での運用可否**: Raspberry Pi 4では最小モデルでも厳しい一方、QNAP NAS（RAM 8GB以上）なら実用的な運用が可能であると分析しました。

## Key Decisions
- QNAP上のDockerでOllamaを使用し、Gemma 4 E2Bを試行することを推奨
