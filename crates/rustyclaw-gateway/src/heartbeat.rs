                ...
                if let Ok(file) = File::open(&path) {
                    ...
                    while i < messages.len() {
                        if messages[i].role == "user" {
                            ...
                            for msg in messages.iter().take(next_user_idx).skip(i + 1) {
        ...
        if treat_as_silent {
            ...
            let (target_session_id, channel_id) = if let Some(ref ch_id) = self.home_channel_id {
                ...
                if let Some((_, _, ref session_id)) = active_session {
                    ...
                    if let Some(stripped) = session_id.strip_prefix("discord-C")
                        && let Some(dash_idx) = stripped.find('-')
                    {
                        channel_id = stripped[..dash_idx].to_string();
                    }
